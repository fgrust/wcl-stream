...
struct OBSStudioAPI : obs_frontend_callbacks {
	...
	{
		for (int i = 0; i < main->scenes->count(); i++) {
			QListWidgetItem *item = main->scenes->item(i);
		...
		struct obs_frontend_source_list *sources) override
	{
		for (int i = 0; i < main->transitions->count(); i++) {
			OBSSource tr = main->transitions->itemData(i)
					       .value<OBSSource>();
	...
	{
		return /*main->ui->transitionDuration->value()*/0;
	}
	...
	{
		return (void *)main->ui->actionToggleMacOutput;
	}
	...
	void *obs_frontend_start_output_qaction() override
	{
		return (void *)main->ui->actionStartOutput;
	}

	void *obs_frontend_stop_output_qaction() override
	{
		return (void *)main->ui->actionStopOutput;
	}
