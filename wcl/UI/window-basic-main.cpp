	...
	: OBSMainWindow(parent), ui(new Ui::OBSBasic)
{
	...
	connect(ui->audioActionButton, SIGNAL(clicked()), this,
		SLOT(handleAudioOutput()));

	connect(ui->resetButton, SIGNAL(clicked()), this, SLOT(handleResetSource()));
...
void OBSBasic::Save(const char *file)
{
	...
	obs_data_t *saveData = GenerateSaveData(
		sceneOrder, quickTrData, /*ui->transitionDuration->value()*/ 0,
...
void OBSBasic::LoadSceneListOrder(obs_data_array_t *array)
{
	...
	for (size_t i = 0; i < num; i++) {
		...
		ReorderItemByName(scenes, name, (int)i);
...
void OBSBasic::LogScenes()
{
	...
	for (int i = 0; i < scenes->count(); i++) {
		QListWidgetItem *item = scenes->item(i);
...
#undef SET_VISIBILITY
...

	{
		...
		ui->statusbar->AttachSource(source);
...
#ifndef __APPLE__
		typedef struct enum_device {
			string name;
			string id;
			enum_device(const char *name_, const char *id_)
				: name(name_), id(id_)
			{
			}
		} enum_device_t;

		list<enum_device_t> *audio_devices = new list<enum_device_t>();

		auto enum_devices = [](void *param, const char *name,
				       const char *id) {
			std::list<enum_device_t> *cb =
				(std::list<enum_device_t> *)param;
			enum_device_t dv(name, id);
		...
		obs_enum_audio_monitoring_devices(enum_devices, audio_devices);

		const char *vAudioInputName = "CABLE Input (WCL-Audio Virtual Cable)";
		
		for (list<enum_device_t>::iterator it = audio_devices->begin();
		     it != audio_devices->end(); ++it) {
			if (!it->name.compare(vAudioInputName)) {
				obs_set_audio_monitoring_device(it->name.c_str(),
								it->id.c_str());
				break;
			}
		}
...
OBSScene OBSBasic::GetCurrentScene()
{
	...
	int a = scenes->count();
	QListWidgetItem *item = scenes->item(0);
...
{
	return sources->Get(GetTopSelectedSourceItem());
}
...
void OBSBasic::UpdateSceneSelection(OBSSource source)
{
	if (source) {
		...
		QList<QListWidgetItem *> items =
			scenes->findItems(QT_UTF8(name), Qt::MatchExactly);

		if (items.count()) {
			sceneChanging = true;
			scenes->setCurrentItem(items.first());
			sceneChanging = false;

			OBSScene curScene =
				GetOBSRef<OBSScene>(scenes->currentItem());
...
void OBSBasic::handleVirtualOutput()
{
	...
	if (ui->cameraActionButton->isChecked()) {
		...
		emit ui->actionStopOutput->trigger();
	}
#endif // __APPLE__
}

void OBSBasic::handleAudioOutput()
{
	OBSSceneItem item = GetCurrentSceneItem();
	OBSSource source = obs_sceneitem_get_source(item);
	obs_source_set_muted(source, !ui->audioActionButton->isChecked());
}

void OBSBasic::handleResetSource()
{
	OBSSceneItem item = GetCurrentSceneItem();
	OBSSource source = obs_sceneitem_get_source(item);
	OBSData setting = obs_source_get_settings(source);
	...
	QString sourceInputUrl = ui->sourceUrl->text();

	if (!sourceInputUrl.compare(sourceUrl)) {
		return;
	}

	obs_data_set_string(setting, "input", sourceInputUrl.toLocal8Bit().data());
	const char *setting_json_2 = obs_data_get_json(setting);
...
{
	if (scene != GetCurrentScene() || sources->IgnoreReorder())
		return;

	sources->ReorderItems();
	SaveProject();
}
...
{
	return /*ui->transitionDuration->value()*/ 0;
}
...
{
	QListWidgetItem *item = scenes->currentItem();
	Qt::ItemFlags flags = item->flags();

	item->setFlags(flags | Qt::ItemIsEditable);
	scenes->editItem(item);
	item->setFlags(flags);
}
...
{
	int idx = scenes->currentRow();
	if (idx == -1 || idx == invalidIdx)
		return;

	sceneChanging = true;

	QListWidgetItem *item = scenes->takeItem(idx);

	if (!relative)
		idx = 0;

	scenes->insertItem(idx + offset, item);
	scenes->setCurrentRow(idx + offset);
...
{
	ChangeSceneIndex(true, 1, scenes->count() - 1);
}
...
{
	ChangeSceneIndex(false, scenes->count() - 1,
			 scenes->count() - 1);
}
...
void OBSBasic::CreateSourcePopupMenu(int idx, bool preview)
{
	...
	if (idx != -1) {
		...
		//obs_source_t *source = obs_sceneitem_get_source(sceneItem);
		uint32_t flags = /*obs_source_get_output_flags(source)*/ 0;
			       ...
			       QAbstractItemDelegate::EndEditHint endHint)
{
	...
	obs_source_t *source = obs_scene_get_source(scene);
	RenameListItem(this, scenes, source, text);
...
inline void OBSBasic::OnDeactivate()
{
	...
	if (!outputHandler->Active()) {
		...
		if (trayIcon)
			trayIcon->setIcon(QIcon::fromTheme(
				"obs-tray", QIcon(":/res/images/wcl.png")));
	} else if (trayIcon) {
		if (os_atomic_load_bool(&recording_paused))
			trayIcon->setIcon(QIcon(":/res/images/obs_paused.png"));
		else
			trayIcon->setIcon(
				QIcon(":/res/images/wcl.png"));
	}
}
...
int OBSBasic::GetTopSelectedSourceItem()
{
	...
	return /*selectedItems.count() ? selectedItems[0].row() : */ 0;
}
...
void OBSBasic::SystemTrayInit()
{
	trayIcon.reset(new QSystemTrayIcon(
		QIcon::fromTheme("obs-tray", QIcon(":/res/images/wcl.png")),
		this));
	trayIcon->setToolTip("WCLive");
...
{
	int i = 0;
	SourceTreeItem *treeItem = sources->GetItemWidget(i);
	OBSSceneItem item = sources->Get(i);
	int64_t id = obs_sceneitem_get_id(sceneItem);
	while (treeItem && obs_sceneitem_get_id(item) != id) {
		i++;
		treeItem = sources->GetItemWidget(i);
		item = sources->Get(i);
	}
	if (treeItem)
		return treeItem;
...
QAction *OBSBasic::AddDockWidget(QDockWidget *dock)
{
	...
	bool lock = true; // ui->lockUI->isChecked();
	QDockWidget::DockWidgetFeatures features =
		lock ? QDockWidget::NoDockWidgetFeatures
		     : (QDockWidget::DockWidgetClosable |
			QDockWidget::DockWidgetMovable |
