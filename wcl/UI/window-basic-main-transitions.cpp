...
{
	transitions->blockSignals(true);
	std::vector<OBSSource> transitions_new;
	size_t idx = 0;
	const char *id;
	...
	while (obs_enum_transition_types(idx++, &id)) {
		...
		if (!obs_is_source_configurable(id)) {
			...
			InitTransition(tr);
			transitions_new.emplace_back(tr);
	...
	if (transitions->count())
		transitions->insertSeparator(transitions->count());

	for (OBSSource &tr : transitions_new) {
		transitions->addItem(QT_UTF8(obs_source_get_name(tr)),
					 QVariant::fromValue(OBSSource(tr)));
	}
	transitions->blockSignals(false);
}
...
obs_source_t *OBSBasic::FindTransition(const char *name)
{
	for (int i = 0; i < transitions->count(); i++) {
		OBSSource tr = transitions->itemData(i).value<OBSSource>();
				 ...
				 bool black, bool manual)
{
	...
	if (force) {
		...
		int duration = /*ui->transitionDuration->value()*/0;
...
void OBSBasic::SetTransition(OBSSource transition)
{
	...
	if (oldTransition && transition) {
		...
		if (transition != GetCurrentTransition())
			SetComboTransition(transitions, transition);
		obs_set_output_source(0, transition);
		obs_transition_swap_end(transition, oldTransition);
	} else {
		obs_set_output_source(0, transition);
...
{
	return transitions->currentData().value<OBSSource>();
}
...
void OBSBasic::on_transitions_currentIndexChanged(int)
{
	...
	if (transition)
		...
		AddTransition(transitions->currentData().value<QString>());
}
...
void OBSBasic::on_transitionRemove_clicked()
{
	...
	int idx = transitions->findData(QVariant::fromValue<OBSSource>(tr));
	if (idx == -1)
		return;
...
void OBSBasic::RenameTransition()
{
	...
	if (accepted) {
		...
		int idx = transitions->findData(variant);
		if (idx != -1) {
			transitions->setItemText(idx,
						     QT_UTF8(name.c_str()));
...
void OBSBasic::SetCurrentScene(OBSSource scene, bool force)
{
	...
	if (obs_scene_get_source(GetCurrentScene()) != scene) {
		for (int i = 0; i < scenes->count(); i++) {
			...
			if (source == scene) {
				scenes->blockSignals(true);
				scenes->setCurrentItem(item);
				scenes->blockSignals(false);
				if (api)
					api->on_event(
...
void OBSBasic::AddQuickTransition()
{
	...
	OBSSource transition =
		fadeToBlack ? OBSSource(fadeTransition)
			    : GetTransitionComboItem(transitions, trIdx);
...
void OBSBasic::QuickTransitionChange()
{
	...
	if (qt) {
		OBSSource tr = fadeToBlack
				       ...
				       : GetTransitionComboItem(transitions,
								trIdx);
		if (tr) {
			qt->source = tr;
			ResetQuickTransitionText(qt);
...
{
	obs_data_array_t *transitions_new = obs_data_array_create();
	...
	for (int i = 0; i < transitions->count(); i++) {
		...
		obs_data_array_push_back(transitions_new, sourceData);
...
void OBSBasic::LoadTransitions(obs_data_array_t *transitions_p)
{
	size_t count = obs_data_array_count(transitions_p);

	for (size_t i = 0; i < count; i++) {
		obs_data_t *item = obs_data_array_item(transitions_p, i);
