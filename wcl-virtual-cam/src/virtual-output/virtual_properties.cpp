    ...
    ui(new Ui::VirtualProperties)
{
	...
	ui->comboBox_target->addItem("WCL-Camera", ModeVideo);
	ui->comboBox_target->addItem("WCL-Camera2", ModeVideo2);
	ui->comboBox_target->addItem("WCL-Camera3", ModeVideo3);
	ui->comboBox_target->addItem("WCL-Camera4", ModeVideo4);
...
{
	blog(LOG_INFO, "Virtual output starting ===");
	ShowWarning(false);
	UpdateParameter();
	EnableOptions(false);
